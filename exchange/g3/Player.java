...
public class Player extends exchange.sim.Player {
    ...
    public Offer makeOffer(List<Request> lastRequests, List<Transaction> lastTransactions) {
        ...
        socksCollection.reset(socks);             
//        socksCollection.sortSocks();
        
        int[] worstPair = this.socksCollection.getWorstPair();
        if(worstPair[0] == -1 || worstPair[1] == -1)
            return new Offer(null, null);
        
        id1 = Math.max(0, worstPair[0]);
        id2 = Math.max(0, worstPair[1]);
